...
. .\subsystem\Get-customerProjectLeadEmail.ps1
...
Write-Host "This script will go over all" $awsAccountCollectionCounter "HEC AWS subscriptions and show a list of certificates that will expire soon."
...
$awsAccountCollection | ForEach-Object { ## awsAccountCollection.GetEnumerator()
    ...
    $accountCounter++
    $certsFound=$false
    $awsAssumeRole="XYZ_CAM_Role_Readonly"
    ...
    if ($currrentRole.Arn -notlike "arn:aws:iam::accountnumber:user/ecs_cli_ro_access") { #AWS master account means assume role did not succeed. Go to next account.
